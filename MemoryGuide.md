...
2.  **总结设置**
    *   **大总结**：v2.3.2 起改为手动滚动合并。待合并微言录达到提醒块数，或活动史册接近输入 Token 上限时，进入“大总结（合并精炼）”处理。
    *   **小总结设置**：
        *   交互式巡录：🟢 **开启**
        *   后台自动总结：🟢 **开启**
        *   存世界书：🟢 **开启**
...
3.  **活动史册合并（v2.3.2）**
    *   **路径**：`Amily2插件` → `总结模块` → `大总结（合并精炼）`。
    *   **快捷操作**：点击 `合并当前活动史册`。插件会读取当前激活的史册，把“既有滚动宏史卷 + 最早的一段待合并微言录”送入预览；确认后才会写回。
    *   **分段原则**：一次点击最多调用模型一次，并只选择完整消息预算内最早、最长的连续微言录前缀。若已有三四百个总结块，完成一段后继续手动点击即可；未选中的后续微言录会原样保留。
    *   **不会自动烧 Token**：达到提醒块数只弹出提示，不会后台自动发起大总结，也不会自动循环处理剩余块。
    *   **默认安全值**：

        | 参数 | 默认值 | 说明 |
        | :--- | :--- | :--- |
        | 合并提醒块数 | `40` | 待合并微言录达到该数量时只提醒 |
        | 史册/重铸输入上限 | `48000 Token` | 对完整消息做保守估算；调用前超限即停止 |
        | 活动宏史卷上限 | `12000 Token` | 模型结果超限时不写入，返回预览继续缩减 |

        输入上限始终至少比活动宏史卷上限多预留 `4000 Token`，用于提示词和本轮微言录证据。这里是保守估算，不等同于服务商的精确 tokenizer 计数。

    *   **可恢复写入**：每次覆盖前都会在同一世界书保存一份禁用的完整正本，可在上方 `总结归档与恢复` 中回溯。保存失败、目标在处理期间变化或输出包含伪造的史册保留标记时，不会覆盖原史册。
    *   **归档需自行管理**：禁用正本不会被插件自动删除，会随手动合并次数增加；确认无需回退后可在世界书中手动清理。
    *   **旧向量占位史册**：若旧史册已把早期正文替换成“由翰林院向量化注入”等占位文字，插件会拒绝继续压缩。请先从完整正本恢复，避免在缺失历史证据时继续合并。
