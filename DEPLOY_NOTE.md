...
- **非公开核心发布门禁**：内测功能、认证密钥与关键权限策略统一进入加密混淆清单；保护目录新增源码但未登记时，测试与发布流水线都会拒绝继续
...
- 第三方数据库脚本兼容迁移、渐进记忆正式验收、战斗系统产品化与时间河均不属于 v2.3.0

### 发布前验证

- 安全回归测试 `39/39`、表格一期测试 `32/32`、时间河既有回归测试 `6/6` 全部通过
- 本次改动的 JavaScript 文件通过真实 ES module 解析，版本 JSON、差异格式、敏感信息扫描及非公开源码清单校验通过
- 代码与本地自动化门禁已完成；真实 SillyTavern 浏览器联调由 `2603` 测试环境继续验收
